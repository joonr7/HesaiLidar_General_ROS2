...
struct PointXYZIT {
  PCL_ADD_POINT4D   // Add xyz in pcl
  float intensity;
  double timestamp;
  uint16_t ring;                   ///< laser ring number
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW  // make sure our new allocators are aligned, Make sure to define a new type of point cloud memory aligned with SSE
} EIGEN_ALIGN16;                   // Force SSE padding to align memory correctly
